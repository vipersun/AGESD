...
def pretrain_agesd(dataset):
    model = AGESD(num_features=args.input_dim, hidden_size=args.hidden1_dim,
    ...
    for epoch in range(args.max_epoch):
        ...
        with torch.no_grad():
            ...
            kmeans = KMeans(n_clusters=args.n_clusters, n_init=20, init='k-means++').fit(z.data.cpu().numpy())      #random
           
        if epoch % (args.max_epoch-1) == 0 and epoch != 0:
            torch.save(model.state_dict(), 'preagesd.pkl')
            print('Save .pkl file!')
